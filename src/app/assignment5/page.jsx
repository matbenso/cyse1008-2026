...
export default function Assignment5Page() {
  ...
  return (
    ...
    >
      ...
      <p style={{ color: "#555" }}>
        Click each button, then open <strong>DevTools → Console</strong> (F12)
        to see the logs. The JSON response also appears in the box below each
        button.
      </p>
      ...
      <Section title="Part B — Step 2: POST request" color="#2e7d32">
        <p>
          A <code>POST</code> request sends data <em>to</em> the server inside a
          JSON body. The same route handles both GET and POST — notice the
          exported function names.
        </p>
        <Button
          label='Call POST /api/assignment5  (body: { name: "..." })'
          onClick={handlePost}
        />
      ...
      <Section title="Part B — Step 3: Callback route" color="#6a1b9a">
        <p>
          This simulates a <strong>webhook callback</strong> — as if Stripe were
          calling our API to say a payment completed. In the real app we never
          call this button; Stripe sends the POST automatically after a customer
          pays. Route: <code>src/app/api/assignment5/callback/route.js</code>
        </p>
        <Button
          label='Simulate callback POST  (type: "payment.completed")'
      ...
      <Section
        title="Part B — Step 4: Fetch products from Firestore"
        color="#e65100"
      >
        <p>
          This route connects to Firestore on the <em>server</em>, fetches the{" "}
          <code>products</code> collection, then loops through each document to
          count total stock and calculate total inventory value. Route:{" "}
          <code>src/app/api/assignment5/products/route.js</code>
        </p>
        <Button
          label="Call GET /api/assignment5/products"
        ...
        {productsResult && (
          ...
          >
            <strong>Summary:</strong> {productsResult.count} products
  ...
  if (!data)
    return (
      <p style={{ color: "#999", fontSize: 13 }}>
        No response yet — click the button above.
      </p>
    );
  return (
    <pre
